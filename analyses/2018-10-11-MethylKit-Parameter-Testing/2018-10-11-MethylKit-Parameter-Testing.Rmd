...
```{r}
processedFilesCov3 <- processBismarkAln(location = analysisFiles, sample.id = sample.IDs, assembly = "v3", read.context = "CpG", mincov = 1, treatment = treatmentSpecification) #Process files for CpG meetehylation using 1x coverage. First 5 files were from ambient conditions, and the second from high pCO2 conditions.
```
...
fileName$actualFileName5 <- paste(fileName$nameBase, "-5xCoverage", "-Sample", fileName$sample.ID, ".jpeg", sep = "") #Create a new column for the full filename for 3x coverage + specific sample's percent CpG methylation plot
fileName$actualFileName6 <- paste(fileName$nameBase2, "-5xCoverage", "-Sample", fileName$sample.ID, ".jpeg", sep = "") #Create a new column for the full filename for 3x coverage + specific sample's percent CpG coverage plot
...
#write.csv(diffMethStats50Cov1, "2018-10-11-Differentially-Methylated-Loci-50-Cov1.csv") #Save table as .csv
```
...
#write.csv(diffMethStats50Cov3, "2018-10-11-Differentially-Methylated-Loci-50-Cov3.csv") #Save table as .csv
```
...
#write.csv(diffMethStats50Cov5, "2018-10-11-Differentially-Methylated-Loci-50-Cov5.csv") #Save table as .csv
```